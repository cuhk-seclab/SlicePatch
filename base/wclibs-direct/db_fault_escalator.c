...
void sqli_error_check (unsigned char *cptr, size_t len);
...
void sqli_error_check (unsigned char *cptr, size_t len) {
  ...
  if (pattern_in_bytes(cptr, len, mysql_msg, strlen(mysql_msg))){
    // Create sqli.log file when SQL injection is detected
    FILE *sqli_log = fopen("/tmp/sqli.log", "w");
    if (sqli_log) {
      fprintf(sqli_log, "[SQLI_DETECTED] ");
      print_repr(sqli_log, cptr, len);
      fprintf(sqli_log, "\n");
      fclose(sqli_log);
    }

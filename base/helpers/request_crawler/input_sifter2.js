...
//        console.log("Starting formdatafrompage");
    ...
    async do_login(page){
        ...
        try {
            ...
            await page.screenshot({path: '/p/tmp/screenshot-post-login.png', type:"png"});
    ...
    async start() {
        ...
        try {
            try{
                this.browser = await puppeteer.launch({headless:this.appData.headless, args:["--disable-features=site-per-process", "--window-size=1600,900", "--no-sandbox"], "defaultViewport": null }); //
                //console.log("OPENED BROWSER!");
                this.browser_up = true;
            } catch (xerror) {
                //console.log("UNABLE TO OPEN X DISPLAY");
                if (xerror.message.indexOf("Unable to open X display") > -1){
                    this.browser = await puppeteer.launch({headless:this.appData.headless, args:["--disable-features=site-per-process", "--no-sandbox"] });
                    this.browser_up = true;
                } else {
                    this.browser_up = false;
